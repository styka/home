...
2. **Automatyczne przejścia + zawracanie.** Nie wpisujesz kolejnych komend. Po `/specify` pipeline
   **sam** przetacza się przez plan → zadania → implementację → weryfikację → recenzję, aż do merge do
   `develop`, a na końcu **automatycznej promocji `develop → master`** (produkcja). `/review` **nie
   czeka na twój approve** — sam wystawia werdykt. Gdy `/verify` albo `/review` znajdzie brak — **sam
   zawraca do `/implement`** i poprawia. Na samym końcu, po pushu `develop`, pipeline **automatycznie**
   merguje `develop → master` i pushuje na produkcję (bez pytania — z góry autoryzowane, `C-52`),
   z kontrolą integralności, żeby nigdy nie cofnąć produkcji.
...
| 6. Recenzja | `/review <slug>` | diff | `review.md` | Świeże oko: poprawność + konwencje Omnia. Sam wystawia werdykt (bez twojego approve), po APPROVE robi merge do `develop`, a następnie **automatycznie** promuje `develop → master` (produkcja) — bez pytania. |
...
3. Jeśli coś wymaga decyzji — dostaniesz **jeden** ekran wyboru. Kliknij rekomendowaną opcję
   ...
   dalej nie ma już żadnego pytania, także o produkcję.
...
6. Efekt oglądasz w `specs/001-portfel-csv-export/` (komplet artefaktów), na `worldofmag.onrender.com`
   (test) i na `omnia-prod.onrender.com` (produkcja).
