...
## Jak to działa w praktyce — dwie zasady UX
1. **Jeden moment pytań.** Pipeline zapyta cię o cokolwiek **tylko raz**, na starcie (`/specify`),
   w **jednym** ekranie wyboru. Rekomendowana odpowiedź jest zawsze **pierwsza** i oznaczona
   `(zalecane)` — zwykle to jest właściwy wybór, więc wybór zajmuje sekundę. Dalsze etapy **nie
   dopytują**: jadą na twoich odpowiedziach i rozsądnych domyślnych.
2. **Automatyczne przejścia.** Nie musisz wpisywać kolejnych komend. Po `/specify` pipeline **sam**
   przetacza się przez plan → zadania → implementację → weryfikację → recenzję, aż do merge do
   `develop`. Jeśli `/verify` znajdzie brak — sam zawraca do `/implement` i poprawia. Zatrzyma się i
   zapyta tylko przy realnym ryzyku nieodwracalnej szkody.
