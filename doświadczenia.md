...
**Rozwiązanie:** W nowym raporcie zrezygnowałem z `>` na rzecz akapitów zaczynających się od **pogrubionej** etykiety (np. `**Ważna uwaga:** ...`). Treść używa tylko realnie wspieranej składni: `#`/`##`/`###`, tabele z wierszem-separatorem, bloki/kod inline, listy `- `, `**bold**`/`*italic*`/`[link]()`, `---`. Listy numerowane (`1.`) i `####` też nie działają (renderują się jako akapit) — pominięte.
**Lekcja:** Renderer Markdown w repo jest własny i okrojony; kolejność transformacji ma znaczenie (escape `<`/`>`/`&` poprzedza parsowanie cytatów, więc `>`-cytaty są martwe). Pisząc treść raportów trzymaj się sprawdzonego podzbioru składni i weryfikuj nietypowe konstrukcje względem `src/lib/markdown.ts`, zanim wrzucisz je do migracji. Docelowo warto naprawić renderer (parsować cytaty przed escapowaniem albo dopuścić `&gt; ` w regexie).
