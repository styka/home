...
- [ ] **T-7** — **`resolver.ts`: przeniesienie wysiłku.** `ResolvedLlm.effort`; gałąź przypisania
...
- [ ] **T-8** — **`llmConfig.ts`: DTO + zapis + walidacja.** `AssignmentDTO` dostaje `effort` i
...
- [ ] **T-9** `[P]` — **Trzeci poziom asystenta w typach.** `AssistantLevel` + `"max"`,
...
- [ ] **T-10** — **`chat.ts`: wysiłek w obu ciałach żądania.** `ChatOptions.effort` + `boostEffort`;
...
- [ ] **T-11** — **Degradacja przy 400.** Gdy żądanie z wysiłkiem dostanie 400 rozpoznany przez
...
- [ ] **T-12** — **Parser odpowiedzi Anthropic przy rozszerzonym myśleniu.** Sprawdź, jak
...
- [ ] **T-13** — **`usage.ts`: wysiłek w logu wywołań.** `AiCallEntry.effort` → kolumna
...
- [ ] **T-14** — **`operationTypes.ts`: obsługa `max`.** `effectiveOperation`: `economy` → `dispatch`
  (bez zmian), `max` → `op` bez zmian; nowy `effortForLevel(base, level)`.
  *Gotowe, gdy:* testy potwierdzają oba przejścia.

- [ ] **T-15** — **`agent/route.ts`: tryb `max`.** W trybie maksymalnym `primaryOp` to **zawsze**
...
- [ ] **T-16** — **Panel administratora: trzy kontrolki.** W `AssignmentRow` drugi wiersz siatki:
...
- [ ] **T-17** — **Informacja o możliwościach dostawcy.** Pod kontrolkami komunikat wyliczany na
...
- [ ] **T-18** `[P]` — **Trzecia opcja w przełączniku asystenta.** Ikona i kolor dla „Maksymalny"
...
- [ ] **T-19** — **Bramki lokalnie.** `npm run check:migrations`, `npm run check:actions`,
...
- [ ] **T-20** — **Dokumentacja.** `CLAUDE.md`: `effort` w opisie konfiguracji LLM (wraz z informacją,
  że możliwości zależą od rodziny modelu) + trzeci poziom pracy asystenta.
  *Gotowe, gdy:* opis zgadza się z kodem.

- [ ] **T-21** — **Mapowanie AC → wynik** (input do `/verify`): dla każdego z AC-1..AC-15 wskazanie
  zadania i sposobu sprawdzenia.
  *Gotowe, gdy:* żaden AC nie zostaje bez pokrycia.

- [ ] **T-22** — **Wpis(y) do `doświadczenia.md`** (C-51), jeśli po drodze wyjdzie nieoczywisty
...
## Notatki / blokady
- **T-12 jest jedynym zadaniem rozpoznawczym** — plan świadomie zostawia rozstrzygnięcie na kod, bo
