...
- [x] **T-1** — `schema.prisma`: model `TaskArea` (projectId→Cascade, parentId self-relation
...
- [x] **T-2** — Czyste funkcje `src/modules/tasks/lib/obszary.ts`: budowa drzewa z płaskiej listy,
...
- [x] **T-3** — Akcje `src/modules/tasks/actions/obszary.ts`: `getProjectAreas` (komentarz
...
- [x] **T-4** — `updateTask` przyjmuje `areaId` (walidacja przynależności obszaru do projektu
...
- [x] **T-5** — `TrashStatus` (union) + zamiana 6 miejsc kasujących `TrashItem` na statusy:
...
- [x] **T-6** — Refaktor: restoratory + dispatch z `src/actions/trash.ts` →
  ...
  (smoke).
- [x] **T-7** — Admin: `src/actions/adminTrash.ts` (`getAdminTrash` z kursorem i filtrami,
  `adminRestoreTrashItem` + `logAudit`), `AuditCategory` += `"admin"`. Wpisy coverage
  (`access: "admin"`). Gotowe, gdy: `npm run check:ai-coverage` OK.

## Faza 3 — UI
- [x] **T-8** — `WyborObszaru.tsx` (dropdown z wcięciami) wpięty w `TaskDetail` i
  `FormularzZadania`; teksty w `messages/pl.json`. Gotowe, gdy: przypisanie/odpięcie obszaru
  działa w dev (AC-2).
- [x] **T-9** — Stan widoku: `layout` += `"obszary"`, parametr `obszary: sekcje|drill|panel`,
...
- [x] **T-10** — `ObszaryWidok.tsx` + `ObszarySekcje.tsx` (wariant domyślny): sekcje wg drzewa,
...
- [x] **T-11** `[P]` — `ObszaryDrill.tsx` (mobile: kafle pod-obszarów + zadania + okruszki) i
...
- [x] **T-12** `[P]` — `/trash`: etykieta modułu `"obszary"`; przywrócenie obszaru z kosza działa
  (AC-6).
- [x] **T-13** — Panel `/admin/kosz`: `page.tsx` + `KoszAdmina.tsx` (tabela, filtry, „Przywróć
...
- [x] **T-14** — Pełna lokalna weryfikacja: `npm run check:pagination`, `check:i18n`,
  ...
  wszystko zielone.
- [x] **T-15** — Mapowanie AC-1…AC-9 na wyniki (input do `/verify`): smoke w dev wg planu §8,
  w tym AC-7 (statusy w DB po opróżnieniu/retencji) i AC-9 (projekt zespołowy).
- [x] **T-16** — Wpis(y) do `doświadczenia.md`, jeśli był nieoczywisty problem (C-51); commit.
...
- Korekty względem planu (C-54, minimalizm C-53):
  - T-8: wybór obszaru wpięty w `TaskDetail` (edycja zadania — AC-2); `FormularzZadania` świadomie
    pominięty — formularz tworzenia wybiera projekt dynamicznie, a obszar przypina się w szczegółach.
  - T-13: wpis w `view-contract.json` niepotrzebny — bramka `check:ui-contract` wyłącza katalog
    `admin` (NOT_MODULES), wystarczył rejestr `narzedzia.ts` (+ test kluczy) i `check:admin-links`.

## Wynik bramek (T-14, 2026-08-31)
- Pełny `npm run build` (lokalny Postgres): **zielony** — wszystkie bramki, `tsc`, `next lint`
  (0 warningów), `next build`, `check:perf`, `migrate.js` (lokalna baza).
- `npm run test:unit` z `DATABASE_URL`: 1504/1504 pass (w tym 6 nowych testów drzewa obszarów).
- Smoke AC (skrypt na lokalnej bazie, wspólny restorator): AC-5 (poddrzewo → 0 obszarów,
  `areaId=null`), AC-6 (restore odtwarza 3 węzły + przypisania), AC-7 (wpis po „opróżnieniu"
  istnieje ze statusem `emptied`), AC-8-rdzeń (restore z wpisu `emptied` działa) — wszystko OK.
- Po drodze naprawione dwie czerwienie odziedziczone z develop po merge 116 (opisane w
  `doświadczenia.md`): brak wyjątku `ownership-scope` dla `skinAssets.ts` i pęknięta zapadka
  `check:domain` (pomocniki 116 wyprowadzone do `src/lib/skins/zapis.ts`).
