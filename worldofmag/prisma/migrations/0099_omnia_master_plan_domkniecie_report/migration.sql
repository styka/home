-- Seed raportu „Omnia — Master plan domknięcia (2026-06-07)" jako raport systemowy.
-- Scala i aktualizuje raporty z 2026-05-31/2026-06-01 do stanu kodu na 2026-06-07.
-- Kategoria "backlog" → rzuca się w oczy w /reports i /admin/reports jako lista zadań.
-- authorId = NULL → raport systemowy. Idempotentnie: ON CONFLICT (slug) DO UPDATE.
...
- `NM1` **Kalendarz** — agreguje Zadania/Posiłki/Zdrowie/Pojazdy/**Leki**, ale **NIE** opiekę nad zwierzętami (`PetCareTask`/`PetTreatment.nextDueAt`) ani SRS języków (`Vocabulary.dueAt`). §18.5 chce „daty ze **wszystkich** modułów". Domknięcie: dorzucić te dwa źródła + spiąć `/pets/calendar`.
...
#### M1 — Czat / wiadomości klient↔wykonawca (P0 · F2) ❌
...
#### M2 — Kalendarz dostępności + rezerwacja slotów (P0 · F2) ❌ [rdzeń Booksy]
...
- **M8 — usługi z czasem trwania + warianty cennika (P1) ❌** [konieczne dla M2]. `ServiceListing.durationMin` + model `ServiceVariant { listingId, name, durationMin, priceAmount }`. *Dlaczego:* rezerwacja slotów wymaga znajomości czasu trwania; usługi mają warianty (np. strzyżenie damskie/męskie).
...
- **M7 — weryfikacja wykonawcy (P1) ❌**. `ServiceProvider.verified`, NIP/dokumenty, badge zaufania. *Dlaczego:* zaufanie to waluta marketplace; zweryfikowany badge podnosi konwersję i odróżnia od anonimowych ofert.
- **M9 — płatności / depozyt / faktury (P1) ❌**. Spięcie z **Portfelem** (wpis `WalletEntry`), opcjonalny depozyt rezerwacyjny, faktura/paragon. *Dlaczego:* domknięcie transakcji w aplikacji (a nie poza nią) buduje przewagę i przychód; integracja z Portfelem to istniejąca synergia.
- **M10 — filtry zaawansowane (P1) ❌**. Cena/ocena/lokalizacja/dostępność/typ oferty. *Dlaczego:* przy wielu ofertach katalog bez filtrów jest bezużyteczny.
- **M12 — reschedule + polityka anulowania (P1) ❌**. Zmiana terminu wizyty, reguły anulowania/okno. *Dlaczego:* Booksy bez przekładania wizyt nie istnieje; redukuje no-show.
- **M18 — onboarding wykonawcy (kreator profilu) (P1) ❌**. Wieloetapowy kreator: profil → kategorie → usługi → dostępność → portfolio. *Dlaczego:* dziś prosty formularz; dobry onboarding = więcej aktywnych wykonawców (podaż napędza marketplace).
