...
`openTask` staje się `null`, a panel szczegółów — który renderuje się warunkowo
...
`tasks`) — zmiana dotyczy wyłącznie trwałości panelu szczegółów.
...
- `prisma/migrations/0109_omnia_implementation_report_task_detail_sticky/migration.sql` — ten raport.
...
→ pojawia się w `/reports` po deployu na środowisko testowe (`develop`).
