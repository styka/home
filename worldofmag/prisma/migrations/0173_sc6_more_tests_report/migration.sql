...
- **2026-06-10 — AI pro H3 ✅.** Transparentnosc asystenta. `chatComplete` (`src/lib/llm/chat.ts`)
  zwraca teraz `model` + `usage` (tokeny; OpenAI `usage`, Anthropic `usage.input/output`). Agent
  (`api/llm/home/agent/route.ts`) sumuje tokeny przez cala petle narzedzi i zwraca `meta`
  (`{model, tokens}`) w body (oba tryby: SSE i JSON). Klient (`AICommandSheet`): `MetaFooter` pod
  kazda odpowiedzia (answer/clarify/navigate/plan) pokazuje uzyty model i liczbe tokenow. Pozostale
  elementy H3 byly juz obecne: historia polecen = watki `AiConversation`; undo = `undoActions` dla
  planow + (nowo) kosz H5 dla usuniec wykonanych przez AI (deleteNote/deleteTask ida przez kosz).

> Nastepna sesja: AI pro H4 (niezawodnosc/rate-limit/degradacja) lub Etap C marketplace
> (M14/M16/M17/M19) lub S6 (ceny zakupow) lub Faza 4.

- **2026-06-10 — AI pro H4 ✅.** Niezawodnosc asystenta. `src/lib/ai/rateLimit.ts`: rate-limit
  per user (sliding window in-memory: 20/min + 250/h) oraz `acquireSlot` (straznik wspolbieznosci,
  max 2 rownolegle ciezkie operacje). Trasa agenta sprawdza limit zaraz po auth (→ 429 + Retry-After)
  i rezerwuje slot przed petla (→ przyjazny 429 „przetwarzam poprzednie polecenie"), zwalnia w
  `finally` obu trybow (SSE/JSON). Graceful degradation: brak klucza → 503 z instrukcja (Admin→LLM),
  limit/blad → komunikat; klient renderuje to istniejacym fallbackiem JSON (nie-SSE). Trwala kolejka
  async + wspoldzielony limiter (Redis/DB) odlozone do SC2/Faza 4 (free-tier = 1 instancja, in-memory
  wystarcza). **AI pro domkniete (H3/H4/H5).**

> Nastepna sesja: Etap C marketplace (M14/M16/M17/M19) lub S6 (ceny zakupow → domkniecie W4/F1)
> lub Faza 4 (SC2-SC7, branze V1-V5, nowe dzialy).

- **2026-06-10 — Zakupy S6 ✅ (domkniecie W4 dla zakupow).** `Item.price` (cena jednostkowa) +
  input „cena zl" w edycji pozycji i wyswietlanie na liscie. Akcja `completeShopping(listId,
  {bookToPortfel})` zastapila `archiveList` w modalu „Zakoncz zakupy": liczy sume kupionych pozycji
  (cena×ilosc dla DONE) i — gdy uzytkownik zaznaczy — ksieguje ja jako wydatek w Portfelu (silnik W4
  `bookAutoExpense` z `force:true`, kategoria „zakupy", sourceModule „shopping"). Modal pokazuje
  „Wydano X zl" + checkbox (tylko gdy konto Portfela skonfigurowane — `financeReady` przekazywane ze
  strony). `bookAutoExpense` dostalo opcje `force` (jawna akcja pomija globalny przelacznik auto,
  ale wciaz wymaga konta). Tym samym W4 obejmuje juz Flote ORAZ Zakupy.

> Nastepna sesja: Etap C marketplace (M14 firma+pracownicy, M16 promocje, M17 moderacja, M19 SEO)
> lub Faza 4 (SC2-SC7, branze V1-V5, nowe dzialy NM2/NM4/NM6-8/NM10) lub pomniejsze (H1, A1/A2, P1-P4).

- **2026-06-10 — Admin A2 ✅.** Szyfrowanie kluczy API w spoczynku + maskowanie wszedzie.
  `src/lib/crypto/secrets.ts`: AES-256-GCM (`encryptSecret`/`decryptSecret`/`maskSecret`/
  `isSecretConfigKey`), klucz wyprowadzony z `CONFIG_SECRET`||`AUTH_SECRET`; WSTECZNIE KOMPATYBILNE
  (wartosci bez prefiksu `enc:v1:` = plaintext, dzialaja do pierwszego ponownego zapisu; zly klucz →
  pusty string, miekka degradacja). Zapisy szyfruja: `setConfigValue` (klucze `*_api_key/_secret/
  _token/_password`), `createProvider`/`updateProvider` (`LlmProvider.apiKey`). Odczyty deszyfruja
  tylko do UZYCIA: resolver (provider + groq), webSearch (brave), ors. UI NIGDY nie dostaje surowego
  klucza: nowy `getConfigMasked` → strona `/admin/config` przekazuje tylko `{hasValue, masked}`
  (usunieto „reveal raw"); `getLlmProviders` maskuje po deszyfracji. Lekcja w doswiadczenia.md:
  `AUTH_SECRET` musi byc staly (rotacja = ponowne wpisanie kluczy).

> Nastepna sesja: Etap C marketplace (M14/M16/M17/M19) lub A1 (audyt RBAC/config) lub Faza 4
> (SC2-SC7, branze V1-V5) lub pomniejsze (H1 personalizacja, P1-P4 Pets).

- **2026-06-10 — Admin A1 ✅.** Dziennik audytu zmian RBAC i konfiguracji. Model `AuditLog`
  (`actorId/actorEmail` migawka, `category` rbac|config, `action`, `target`, `detail`). Helper
  `src/lib/audit.ts/logAudit` (server-side, best-effort — sam pobiera aktora z sesji, blad logowania
  nie wywraca operacji). Wpiety w mutacje RBAC (`createPermission`/`updatePermission`/
  `deletePermission`/`toggleRolePermission`/`addUserRole`/`removeUserRole`) i konfiguracji
  (`setConfigValue` — bez wartosci sekretu, tylko fakt zmiany; `createProvider`/`updateProvider`/
  `deleteProvider`/`setAssignment`). Akcja `getAuditLog` (200 ostatnich, filtr kategorii) +
  przegladarka `/admin/audit` (`AuditLogPage`, zakladki Wszystko/RBAC/Konfiguracja) + wejscie z
  konsoli admina. **Bezpieczenstwo/Admin: A1 i A2 domkniete; zostal A3 (panel zdrowia systemu).**

> Nastepna sesja: Etap C marketplace (M14/M16/M17/M19) lub A3 (panel zdrowia) lub Faza 4
> (SC2-SC7, branze V1-V5) lub pomniejsze (H1 personalizacja, P1-P4 Pets).

- **2026-06-10 — Admin A3 ✅.** Panel zdrowia systemu `/admin/health`. Akcja `getSystemHealth`
  zbiera: stan DB (latencja `SELECT 1`, liczba + nazwa ostatniej migracji z `_prisma_migrations`),
  LLM (dostawcy aktywni/wszyscy, przypisania per typ operacji z flaga OK, legacy Groq), integracje
  (Brave/ORS — sama obecnosc klucza, env lub Config), build info (`NEXT_PUBLIC_BUILD_*`), liczby
  rekordow kluczowych encji, statystyki audytu. UI `SystemHealthPage` (karty: Baza/LLM/Integracje/
  Build/Dane, zielone/czerwone statusy). Wejscie z konsoli admina. **Bezpieczenstwo/Admin domkniete
  (A1 audyt, A2 szyfrowanie, A3 zdrowie).**

> Nastepna sesja: Etap C marketplace (M14/M16/M17/M19) lub Faza 4 (SC2-SC7, branze V1-V5) lub
> pomniejsze (H1 personalizacja dashboardu, P1-P4 Pets, X2/X3 propagacja UI).

- **2026-06-10 — Home H1 ✅.** Personalizacja pulpitu. Model `DashboardPref` (per-user `order` +
  `hidden` jako JSON string[]) + akcje `dashboardPrefs.ts` (`get`/`set`). `HomePage` buduje sekcje
  do mapy `sectionNodes` (recently/briefing/modules/today/quickActions/suggestions) i renderuje wg
  kolejnosci uzytkownika (puste/ukryte pomijane). Przycisk „Dostosuj pulpit" wlacza tryb edycji:
  per-sekcja ↑/↓ (kolejnosc) + 👁 (ukryj/pokaz) z autosave (`startTransition`). Greeting/zaproszenia/
  admin/footer pozostaja stale. Prefs ladowane na serwerze (`getDashboardPrefs`) i przekazane do
  `HomePage`.

> Nastepna sesja: Etap C marketplace (M14/M16/M17/M19) lub Faza 4 (SC2-SC7, branze V1-V5) lub
> pomniejsze (P1-P4 Pets, X2/X3 propagacja design systemu, X5 a11y).

- **2026-06-10 — Marketplace Etap C: M16 ✅ (kody rabatowe).** Model `ServicePromoCode`
  (per-wykonawca, `kind` percent|amount, `value`, `minAmount?`, `maxUses?`, `usedCount`, `active`,
  `expiresAt?`, unikat providerId+code). `ServicePayment` + `promoCode`/`discount` (grosze; netto =
  amount − discount). Akcje: `getMyPromoCodes`/`createPromoCode`/`togglePromoCode`/`deletePromoCode`
  (wykonawca) + `applyPromoCode(requestId, code)`/`clearPromoCode` (walidacja: aktywny, niewygasly,
  limit uzyc, min-kwota; rabat = % lub kwota, cap do kwoty; zmiana kodu cofa zuzycie poprzedniego).
  Ksiegowanie M9 (przychod/wydatek) uzywa kwoty NETTO. UI: `PromoCodesManager` w panelu wykonawcy
  (lista + dodawanie + wlacz/wylacz/usun) oraz `PromoCodeRow` w `PaymentSection` (klient/wykonawca
  wpisuje kod → przekreslona kwota brutto + netto + rabat).

> Nastepna sesja: Etap C dalej (M14 firma+pracownicy, M17 moderacja/spory, M19 SEO) lub Faza 4
> (SC2-SC7, branze V1-V5) lub pomniejsze (P1-P4 Pets, X2/X3, X5 a11y).

- **2026-06-10 — Marketplace Etap C: M17 ✅ (moderacja/spory).** Model `ServiceDispute`
  (`requestId`, `openedById`, `reason`, `description?`, `status` OPEN|RESOLVED|REJECTED, `resolution?`,
  `resolvedAt?`). Akcje: `openDispute(requestId, reason, desc?)` (strona zlecenia, jeden OPEN/zlecenie,
  powiadamia druga strone), `getRequestDisputes` (obie strony widza w watku), `getModerationDisputes`
  (admin, filtr statusu, +dane zlecenia/stron), `resolveDispute(id, RESOLVED|REJECTED, note?)` (admin,
  powiadamia obie strony). UI: `DisputeSection` w `RequestThread` (przycisk „Zglos problem" + lista
  zgloszen ze statusem/rozstrzygnieciem); strona `/services/moderation` (`ModerationPage`, admin —
  zakladki Otwarte/Rozwiazane/Odrzucone, rozstrzyganie z notatka) + wejscie z konsoli admina.

> Nastepna sesja: Etap C dalej (M14 firma+pracownicy, M19 profil/SEO) lub Faza 4 (SC2-SC7,
> branze V1-V5) lub pomniejsze (P1-P4 Pets, X2/X3 propagacja UI, X5 a11y).

- **2026-06-10 — Marketplace Etap C: M19 ✅ (profil publiczny/SEO).** `ServiceProvider.slug` (unikat,
  generowany raz z nazwy przez `slugify`+`uniqueProviderSlug`) i `tagline`. `getProviderPublic`
  przyjmuje teraz id LUB slug. Strona `/services/providers/[providerId]` dostala `generateMetadata`
  (tytul = nazwa+obszar, opis = tagline/bio, OpenGraph). `ProviderPublicPage`: tagline w podtytule +
  przycisk „Udostepnij" (kopiuje link ze slug). Panel wykonawcy: pole tagline + link do publicznego
  profilu (/services/providers/<slug>). **Uwaga:** pelne SEO-indeksowanie (sitemap/robots/crawl) jest
  N/A, bo app wymaga logowania (gotcha #8) — to ewentualny „tryb publiczny" w Fazie 4. Istniejacy
  wykonawcy dostaja slug przy najblizszym zapisie profilu (do tego czasu link po id dziala).

> **Etap C marketplace prawie zamkniety** (M11/M13/M16/M17/M19 ✅; zostalo M14 firma+pracownicy
> i M15 abonamenty=monetyzacja/F4). Nastepna sesja: M14 lub Faza 4 (SC2-SC7, branze V1-V5) lub
> pomniejsze (P1-P4 Pets, X2/X3 propagacja design systemu, X5 a11y).

- **2026-06-10 — Zwierzeta P1 ✅ (weryfikacja — JUZ istnieje).** Progressive disclosure funkcji pro
  byl zaimplementowany: `src/lib/petPresets.ts` (7 presetow per gatunek: companion/reptile_keeper/
  reptile_breeder/aquarium/bird/small_mammal/custom; `suggestedPresetForSpecies`), per-pet
  `featureFlags` (JSON) nadpisywane nad presetem przez `resolveFeatures`. `PetDetailPage` pokazuje
  TYLKO zakladki wlaczonych funkcji (companion ukrywa Terrarium/Akwarium/Hodowla/Genetyka). Kreator
  `PetForm` proponuje preset wg gatunku (i pozwala zmienic), a zakladka „Widoczność"
  (`FeatureSettingsSection` → `updatePetFeatures`) daje preset + per-funkcja checkboxy. Zwykly user
  (pies/kot) nigdy nie widzi sekcji hodowlanych. Odhaczone bez duplikacji kodu (wzorzec K1/Z3).

> Nastepna sesja: M14 (firma+pracownicy) lub Faza 4 (SC2-SC7, branze V1-V5) lub pomniejsze
> (P2-P4 Pets, X2/X3 propagacja design systemu, X5 a11y).

- **2026-06-10 — Zwierzeta P2 ✅ (alerty parametrow terrariow).** `addEnvironmentReading` po zapisie
  odczytu klasyfikuje kazdy podany parametr wzgledem zakresow zbiornika (`classifyValue` z
  `targetRanges` zbiornika lub `DEFAULT_RANGES`) i przy statusie warn/danger wysyla powiadomienie
  (silnik NM3 `notifyUser`): tytul z liczba przekroczen i ikona (🔴 danger / 🟠 warn), body z lista
  „parametr: wartosc (cel: …)"; `dedupeKey` per zbiornik+dzien (max 1 alert dziennie, body z aktualnymi
  przekroczeniami). Wizualne badge poza-zakresem w UI husbandry juz byly — to dodaje AKTYWNY alarm.

> Nastepna sesja: P4 (spiecie /pets/calendar z NM1) lub P3 (eksport pomiarow) lub M14 (firma+
> pracownicy) lub Faza 4 (SC2-SC7, branze V1-V5) lub X2/X3 / X5.

- **2026-06-10 — Zwierzeta P4 ✅ (spiecie /pets/calendar z NM1).** Opieka nad zwierzetami juz byla
  agregowana w `getCalendarEvents` (NM1, module „pets"). Wspolny `/calendar` (`CalendarPage`) zyskal
  FILTR modulu: legenda stala sie klikalna (chip = filtr; „Pokaz wszystkie"), a strona `/calendar`
  czyta `?module=` (walidowane po `MODULE_META`) i przekazuje `initialModule`. `/pets/calendar`
  (agenda zalegle/nadchodzace) dostala akcje „Widok miesieczny" → `/calendar?module=pets`. Tym samym
  opieka nad zwierzetami to jedna warstwa we wspolnym kalendarzu (zamiast osobnego widoku).

> Nastepna sesja: P3 (eksport pomiarow PDF/wet.) lub M14 (firma+pracownicy) lub Faza 4 (SC2-SC7,
> branze V1-V5) lub X2/X3 propagacja design systemu / X5 a11y.

- **2026-06-10 — Zwierzeta P3 ✅ (eksport dla weterynarza).** `src/lib/petExport.ts` (czyste buildery,
  bez zaleznosci): `buildVetCardHtml(pet)` — pelna karta HTML (profil, pomiary, leki/szczepienia,
  wizyty wet., dziennik zdrowia, notatki) z `@media print` do druku → PDF z przegladarki;
  `buildMeasurementsCsv(pet)` — CSV pomiarow (data/waga/dlugosc/BCS/notatka, BOM dla Excela).
  W naglowku `PetDetailPage` przyciski: Drukuj karte (otwiera okno + `print()`) i Eksport pomiarow
  (download CSV). **Modul Zwierzeta domkniety: P1 (presety) / P2 (alerty) / P3 (eksport) / P4
  (kalendarz).**

> Nastepna sesja: M14 (firma+pracownicy) lub Faza 4 (SC2-SC7, branze V1-V5) lub X2/X3 propagacja
> design systemu / X5 a11y.

- **2026-06-10 — Marketplace Etap C: M14 ✅ (firma + pracownicy).** Model `ServiceStaff`
  (pracownik/zasob firmy: name/role/active) + `ServiceAvailability.staffId` (harmonogram per-pracownik,
  null=solo) + `ServiceRequest.staffId` (przypisany pracownik). Akcje: staff CRUD
  (`getMyStaff`/`createStaff`/`updateStaff`/`deleteStaff`), `setAvailability(rules, staffId?)` +
  `getMyAvailability(staffId?)`, `getListingStaff`, `computeSlots`/`getAvailableSlots`/`bookSlot`/
  reschedule z parametrem `staffId` (sloty i kolizje liczone per-pracownik; gdy firma ma aktywnych
  pracownikow, rezerwacja MUSI wskazac osobe). UI: `StaffManager` w panelu (CRUD + wlacz/wylacz),
  `AvailabilityEditor` z selektorem „Cala firma / pracownik", `BookingWidget` z wyborem pracownika,
  pracownik pokazany na karcie zlecenia (`MyRequestsPage`). Solo-wykonawcy bez zmian (staffId null).
  **Tryb „dowolny pracownik" (auto-przypisanie) odlozony** — na razie klient wybiera konkretna osobe.

> **Etap C marketplace zamkniety** (M11/M13/M14/M16/M17/M19 ✅; M15 abonamenty = monetyzacja/F4).
> Nastepna sesja: Faza 4 (SC2-SC7 skala/observability, branze V1-V5) lub cross-cutting (X2/X3
> propagacja design systemu + stany puste, X5 a11y, X4 i18n).

- **2026-06-10 — Cross-cutting X3 (stany ladowania) ✅ czesciowo.** Wspolny komponent `LoadingState`
  (`src/components/ui/home/LoadingState.tsx`): page-level skeleton z animacja shimmer (naglowek +
  N kart) zamiast pustego ekranu przy nawigacji. Dodano route-level `loading.tsx` w 21 dzialach
  (calendar/contacts/flota/habits/health/kitchen/languages/magazynowanie/notes/pets/pogoda/portfel/
  reports/services/shopping/tasks/trash/truck/warsztaty/wiadomosci/settings) — Next.js App Router
  pokazuje skeleton natychmiast podczas pobierania danych strony. To czesc X2/X3 (stany ladowania);
  pelna propagacja design-systemu (eliminacja inline-style) i stany bledow pozostaja.

> Nastepna sesja: Faza 4 (SC2-SC7, branze V1-V5) lub dalej cross-cutting (X2 propagacja design
> systemu, X5 a11y, X4 i18n).

- **2026-06-10 — Faza 4 SC6 (testy jednostkowe) ✅ start.** Zerowy narzut zaleznosci: wbudowany
  `node:test` + `tsx` (juz w repo) — skrypt `npm run test:unit`
  (`node --import tsx --test "src/**/*.test.ts"`). Pierwsze 24 testy dla KRYTYCZNEJ czystej logiki:
  `srs.ts` (SuperMemo-2: pierwszy/drugi/kolejny sukces, niepowodzenie+lapse, podloga ease 1.3),
  `recurrence.ts` (DAILY/WEEKLY z daysOfWeek + zawijanie/MONTHLY/YEARLY/parse), `serviceSlots.ts`
  (generacja slotow, kolizje rezerwacji, pomijanie przeszlosci, walidacja czasu), `wikilinks.ts`
  (ekstrakcja/dedupe, resolve, backlinks). Pliki `*.test.ts` wykluczone z `next build` przez
  tsconfig.exclude, wiec nie wplywaja na bundle. Dalej (SC6 c.d.): stats wykonawcy, parseQuantity,
  medicationSchedule, petEnvironment.

> Nastepna sesja: SC6 c.d. (kolejne testy) lub Faza 4 (SC2 limity/cache AI, SC3 wydajnosc, SC4
> observability) lub cross-cutting (X2/X5/X4) lub branze V1-V5.

- **2026-06-10 — Faza 4 SC6 (testy) c.d.** Dodano 18 testow (lacznie **42**): `parseQuantity`
  (smart-parsing zakupow: „2 butelki mleka", „mleko 500ml", „mleko x2", „3 kg ...", przecinek
  dziesietny, sam tekst), `petEnvironment` (`classifyValue` ok/warn/danger dla min/max + progi
  toksyn, null→ok, nieznany param, custom range nadpisuje, `rangeFor`), `serviceGeo`
  (`haversineKm` 0/Wwa↔Krakow~250km/symetria, `formatDistance` m vs km). Wszystkie czyste funkcje
  bez value-importow „@/" (tsx eliduje type-only), wiec brak problemu z aliasami.

> Nastepna sesja: SC6 c.d. (stats wykonawcy, medicationSchedule) lub Faza 4 (SC2/SC3/SC4) lub
> cross-cutting (X2/X5/X4) lub branze V1-V5.
...
- **M14 firma z wieloma pracownikami ✅** [2026-06-10] — model `ServiceStaff` (pracownik/zasób firmy) + `ServiceAvailability.staffId` (per-pracownik harmonogram) + `ServiceRequest.staffId` (przypisany pracownik). Panel: `StaffManager` (CRUD), `AvailabilityEditor` z selektorem „Cała firma / pracownik", `BookingWidget` z wyborem pracownika (sloty + rezerwacja per-pracownik). *Dlaczego:* salony/warsztaty mają wielu pracowników — to model biznesowy Booksy.
- **M15 abonamenty/pakiety ❌**, **M16 promocje/rabaty/kody ✅** [2026-06-10 — `ServicePromoCode` per-wykonawca (percent|amount, limit/wygaśnięcie/min-kwota), zarządzanie w panelu, kod stosowany do płatności → rabat netto], **M17 moderacja/spory ✅** [2026-06-10 — `ServiceDispute` (strona zlecenia zgłasza, admin rozstrzyga OPEN→RESOLVED/REJECTED), `/services/moderation`], **M19 profil publiczny/SEO ✅** [2026-06-10 — `ServiceProvider.slug`+`tagline`, `generateMetadata` per-wykonawca, profil dostępny po slug/id, przycisk Udostępnij; pełne SEO-indeksowanie N/A dopóki app jest auth-only — F4]. *Dlaczego:* monetyzacja, akwizycja (SEO), bezpieczeństwo transakcji przy skali.
...
- **H1 personalizacja dashboardu** ✅ (P2·F3) [2026-06-10] — model `DashboardPref` (per-user order+hidden JSON); tryb „Dostosuj pulpit" na stronie głównej: kolejność sekcji (↑/↓) + ukrywanie (👁) z autosave, sekcje: Ostatnio używane/Briefing/Moduły/Dziś/Szybkie akcje/Sugestie. *Dlaczego:* power-user chce własnego pulpitu; dziś układ stały.
- **H3 transparentność AI** ✅ (P1·F2) [2026-06-10] — „który model" + licznik tokenów pod każdą odpowiedzią (`chatComplete` zwraca model+usage, agent sumuje przez pętlę → `meta` w body → `MetaFooter`); historia poleceń = wątki `AiConversation`; **undo** = `undoActions` dla planów + kosz (H5) dla usunięć przez AI. *Dlaczego:* zaufanie i kontrola kosztów; agent przestał być „czarną skrzynką".
- **H4 niezawodność AI** ✅ (P1·F2) [2026-06-10] — rate-limit per user (`src/lib/ai/rateLimit.ts`: 20/min + 250/h, in-memory) + strażnik współbieżności (max 2 równoległe) na trasie agenta → przyjazny 429; graceful degradation (brak klucza→503 z instrukcją, limit→komunikat) renderowana przez istniejący fallback klienta. *Dlaczego:* przy skali i kosztach LLM brak limitów = ryzyko awarii/rachunku. (Trwała kolejka async + Redis-limiter = SC2/Faza 4.)
...
- **S6 ceny pozycji → Portfel** ✅ (P1·F2) [2026-06-10] — `Item.price` (cena jednostkowa, input w edycji pozycji); „Zakończ zakupy" liczy sumę kupionych (cena×ilość) i — opcją — księguje ją w Portfelu (silnik W4, `completeShopping`, kategoria „zakupy"). *Dlaczego:* synergia Zakupy↔budżet; domyka auto-wydatki W4 dla zakupów.
...
### 4.6 Zwierzęta
- **P1 progressive disclosure funkcji pro** ✅ (P1·F1) [2026-06-10; zweryfikowane — JUŻ istnieje] — `src/lib/petPresets.ts` (presety per gatunek, `suggestedPresetForSpecies`) + per-pet `featureFlags` nadpisania nad presetem (`resolveFeatures`); zakładki w `PetDetailPage` gated po `resolveFeatures` (companion ukrywa Terrarium/Hodowla/Genetyka); kreator `PetForm` (preset wg gatunku) + zakładka „Widoczność" (`FeatureSettingsSection`→`updatePetFeatures`). *Dlaczego:* moduł obsługuje i kota, i hodowcę gadów; pro przytłacza zwykłego usera.
- **P2 alerty parametrów terrariów** ✅ (P2·F2) [2026-06-10] — `addEnvironmentReading` klasyfikuje każdy odczyt względem zakresów zbiornika (`classifyValue` + `targetRanges`/`DEFAULT_RANGES`) i przy przekroczeniu wysyła powiadomienie (NM3, 🔴 danger/🟠 warn, dedup per zbiornik+dzień). *Dlaczego:* odczyty mają zakresy docelowe, brak aktywnych alarmów.
- **P3 eksport pomiarów (PDF/wet.)** ✅ (P2·F2) [2026-06-10] — `src/lib/petExport.ts`: karta zwierzęcia HTML do druku (→ PDF z przeglądarki: profil/pomiary/leki/wizyty/zdrowie) + CSV pomiarów; przyciski Drukuj/Eksport CSV w nagłówku `PetDetailPage`. *Dlaczego:* dane proszą się o eksport dla weterynarza/kupującego.
- **P4 spięcie `/pets/calendar` z NM1** ✅ (P1·F1) [2026-06-10] — opieka nad zwierzętami jest już w `getCalendarEvents` (NM1); wspólny `/calendar` zyskał filtr modułu (klikalna legenda + `?module=`), a `/pets/calendar` linkuje „Widok miesięczny" → `/calendar?module=pets`. *Dlaczego:* jedna warstwa kalendarza zamiast osobnych.
...
### 4.16 Admin
- **A1 audyt RBAC/config (`AuditLog`)** ✅ (P1·F2) [2026-06-10] — model `AuditLog` (aktor/akcja/cel/szczegół, migawka e-maila), `logAudit` (best-effort) w mutacjach RBAC (permission/role-perm/user-role) i konfiguracji (config.set, llm provider/assignment; bez wartości sekretów), przeglądarka `/admin/audit` (filtr RBAC/konfiguracja, 200 ostatnich). *Dlaczego:* przy zespołach zmiany uprawnień muszą być śledzone.
- **A2 szyfrowanie kluczy API + maskowanie wszędzie** ✅ (P1·F2) [2026-06-10] — AES-256-GCM (`src/lib/crypto/secrets.ts`, klucz z `CONFIG_SECRET`/`AUTH_SECRET`, wstecznie kompatybilne z plaintextem); sekrety szyfrowane w spoczynku (Config `*_api_key`, `LlmProvider.apiKey`), deszyfrowane tylko do użycia (resolver/webSearch/ors); UI nigdy nie otrzymuje surowego klucza — `getConfigMasked` + maska. *Dlaczego:* dziś klucze w `Config` plaintext — ryzyko bezpieczeństwa.
- **A3 panel zdrowia systemu (`/admin/health`)** ✅ (P2·F2) [2026-06-10] — `getSystemHealth`: stan DB (latencja + liczba/ostatnia migracja), LLM (dostawcy aktywni, przypisania per typ operacji, legacy Groq), integracje (Brave/ORS — obecność), build info, liczby rekordów, audyt. Strona `/admin/health` z kartami statusu. *Dlaczego:* operacyjna widoczność przed skalą.

### 4.17 Cross-cutting
- **X2/X3 propagacja design systemu + stany puste/ładowania/błędów + onboarding** 🟡 (P0/P1·F1). [2026-06-10 — **stany ŁADOWANIA ✅**: wspólny `LoadingState` (skeleton) + route-level `loading.tsx` w 21 działach; pozostaje propagacja design-systemu/inline-style i stany błędów]. *Dlaczego:* nierówna dojrzałość modułów psuje wrażenie profesjonalizmu.
...
- **SC6 testy jednostkowe krytycznej logiki** 🟡 [2026-06-10 — `node:test`+`tsx` (0 deps), `npm run test:unit`; **42 testy**: SRS, recurrence, slot-generation, wikilinks, parseQuantity, petEnvironment (klasyfikacja zakresów), serviceGeo (haversine); dalej: stats wykonawcy, medicationSchedule] — SRS, recurrence, stats, routing, slot-generation marketplace. *Dlaczego:* regresje w logice biznesowej są kosztowne; moduł QA daje strukturę.
