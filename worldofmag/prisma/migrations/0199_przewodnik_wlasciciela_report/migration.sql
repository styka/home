-- 0199: Przewodnik właściciela (kroki manualne + decyzje) jako raport admina.
-- Kategoria 'backlog' (lista akcji czekających na właściciela). Idempotentnie:
-- ON CONFLICT DO UPDATE odświeża treść przy re-seedzie; slug globalnie unikalny.
...
- ☐ Zakupy → otwórz listę → **dodaj pozycję** (modal); sprawdź, że modal wyśrodkowany, zamyka się
  Esc i kliknięciem w tło.
- ☐ Kuchnia → **import przepisu z URL** (modal) i Spiżarnia → **edycja pozycji** (modal) — nic nie
  „ucieka" poza ekran, działa na telefonie.
