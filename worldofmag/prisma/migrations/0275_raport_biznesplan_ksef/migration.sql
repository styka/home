-- RAPORT: BIZNESPLAN „PROJEKT SEKUNDA" — narzędzia KSeF dla mikrofirm.
--
-- Zlecenie właściciela (29.08.2026): szczegółowy biznesplan przedsięwzięcia poza projektem
-- Omnia (wkład własny 20 000 zł, cel 300 000 zł zysku po roku), z researchem internetowym,
-- źródłami i cytatami; Omnia służy wyłącznie jako nośnik dokumentu. Raport systemowy,
-- widoczny w /reports dla zalogowanych (właściciel = admin).
--
-- Migracja NIE zmienia kształtu bazy: jeden INSERT z ON CONFLICT DO NOTHING (idempotentny,
-- wzorzec 0271). Treść nie zawiera sekretów.

INSERT INTO "Report" ("id", "title", "slug", "content", "category", "storage", "authorId", "teamId", "createdAt", "updatedAt")
VALUES (
  gen_random_uuid()::text,
  'Biznesplan — narzędzia KSeF dla mikrofirm („Projekt Sekunda")',
  'biznesplan-ksef-mikrofirmy-2026',
  $biznesplan$# Biznesplan „Projekt Sekunda" — narzędzia KSeF dla mikrofirm
