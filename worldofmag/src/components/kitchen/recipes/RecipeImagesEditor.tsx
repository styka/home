...
// Zmniejsza zdjęcie po stronie klienta i zwraca data-URL (JPEG). Trzymamy obrazy jako
// data-URL w bazie (brak zewnętrznego storage) — downscale utrzymuje rozsądny rozmiar.
...
export function RecipeImagesEditor({ recipeId, images, hasAI }: RecipeImagesEditorProps) {
  ...
  async function handleOcr(id: string) {
    ...
    setOcrBusyId(id);
    try {
      const res = await llm.kitchen.ocrText(item.url);
      if (res.error) {
