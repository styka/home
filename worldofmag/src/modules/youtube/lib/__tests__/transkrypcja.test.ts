...
/**
 * 123 v3 — droga `instancja` (publiczne Piped/Invidious). To jest odpowiedź na blokadę IP
 * centrów danych: listy i treść napisów przychodzą z instancji, nie z youtube.com.
 */
import {
  sciezkiZPiped,
  sciezkiZInvidious,
...
const PIPED_STREAMS = JSON.stringify({
  title: "Film",
  subtitles: [
    { url: "https://proxy.example/sub?lang=en", code: "en", autoGenerated: true, mimeType: "text/vtt" },
    { url: "/proxy/sub?lang=pl", code: "pl", autoGenerated: false, mimeType: "text/vtt" },
  ],
});

const INVIDIOUS_CAPTIONS = JSON.stringify({
  captions: [
    { label: "Polish (auto-generated)", languageCode: "pl", url: "/api/v1/captions/abc?label=Polish+%28auto-generated%29" },
...
test("ścieżki z Piped: względne adresy dostają bazę, autoGenerated → automatyczne", () => {
  const s = sciezkiZPiped(PIPED_STREAMS, "https://pipedapi.example");
  assert.equal(s.length, 2);
  assert.equal(s[0].baseUrl, "https://proxy.example/sub?lang=en");
  assert.equal(s[0].automatyczne, true);
  assert.equal(s[1].baseUrl, "https://pipedapi.example/proxy/sub?lang=pl");
  assert.equal(wybierzSciezke(s)?.jezyk, "pl", "wspólna preferencja językowa działa");
  assert.deepEqual(sciezkiZPiped("nie-json", "x"), []);
  assert.deepEqual(sciezkiZPiped('{"error":"pad"}', "x"), []);
});

test("ścieżki z Invidiousa: automatyczność niesie etykieta, adresy są względne", () => {
  const s = sciezkiZInvidious(INVIDIOUS_CAPTIONS, "https://inv.example");
  assert.equal(s.length, 2);
  assert.equal(s[0].automatyczne, true, "„auto-generated\" w etykiecie");
  assert.equal(s[1].automatyczne, false);
  assert.ok(s[0].baseUrl.startsWith("https://inv.example/api/v1/captions/"));
  assert.deepEqual(sciezkiZInvidious("", "x"), []);
});

test("WebVTT składa się w tekst: bez nagłówka i czasów, sąsiadujące duplikaty sklejone", () => {
  assert.equal(tekstZNapisow(VTT), "Dzień dobry to jest test");
});

test("lista instancji z Config nadpisuje wbudowaną; śmieci → wbudowana", () => {
  const wlasna = listaInstancji('["invidious:https://moja.inst/", "piped:https://p.inst"]');
  assert.deepEqual(wlasna, [
    { typ: "invidious", baza: "https://moja.inst" },
    { typ: "piped", baza: "https://p.inst" },
  ]);
  assert.deepEqual(listaInstancji(null), DOMYSLNE_INSTANCJE);
  assert.deepEqual(listaInstancji("nie-json"), DOMYSLNE_INSTANCJE);
  assert.deepEqual(listaInstancji('["ftp://zle", 42]'), DOMYSLNE_INSTANCJE);
});

test("droga instancja ratuje film, gdy YouTube blokuje wszystko (scenariusz Rendera)", async () => {
  const diagnoza: string[] = [];
  const wynik = await pobierzTranskrypcje(
    "abc",
    async (url) => {
      if (url.includes("youtube.com")) return null; // blokada IP: nic z YouTube nie odpowiada
      if (url.includes("pierwsza.inst")) return null; // martwa instancja
      if (url.includes("druga.inst/streams/abc")) return PIPED_STREAMS;
      if (url.includes("proxy/sub?lang=pl")) return VTT;
      return null;
    },
    diagnoza,
    [
      { typ: "piped", baza: "https://pierwsza.inst" },
      { typ: "piped", baza: "https://druga.inst" },
    ]
  );
  assert.equal(wynik?.tekst, "Dzień dobry to jest test");
  assert.equal(wynik?.jezyk, "pl");
  assert.equal(wynik?.zrodlo, "instancja");
  assert.ok(diagnoza.some((d) => d.includes("pierwsza.inst")), "martwa instancja odnotowana");
});

test("instancja z poprawną listą jest autorytatywna: pusta lista kończy drogę bez prób kolejnych", async () => {
  ...
  const wynik = await pobierzTranskrypcje(
    ...
    async (url) => {
      ...
      if (url.includes("pierwsza.inst")) return JSON.stringify({ subtitles: [] });
      return PIPED_STREAMS; // druga instancja NIE powinna być pytana
    },
    undefined,
    [
      { typ: "piped", baza: "https://pierwsza.inst" },
  ...
  assert.ok(!odpytane.some((u) => u.includes("druga.inst")), "autorytatywna pusta lista zatrzymuje drogę");
});
